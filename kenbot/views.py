...
@views.route('/metro/<int:msa_md>')
def metro_report(msa_md):
    return render_template('msa_report.html',
                           msa=data.msa(msa_md))


@views.route('/compare')
def msa_compare_index():
    return render_template('compare.html',
                           msas=data.msas())


@views.route('/compare/<int:msa_md_1>/<int:msa_md_2>')
...
# Much of this could be replaced with a single generic query that accepted
# config options to control filtering, pivoting, etc
